...
// 1. 定义 YZTAwakeProtocol 协议
public protocol YZTAwakeProtocol: class {
    static func yztAwake()
}
public protocol YZTFatherAwakeProtocol: class
{   // 1.1 定义 YZTFatherAwakeProtocol ()
    static func fatherAwake()
}

class YZTNothingToSeeHere
{
    static func harmlessFunction(){
        ...
        //            (types[index] as? YZTAwakeProtocol.Type)?.yztAwake() //如果该类实现了SelfAware协议，那么调用 awake 方法
        //            (types[index] as? YZTFatherAwakeProtocol.Type)?.fatherAwake()
        //        }
        //        types.deallocate(capacity: typeCount)
        UINavigationBar.yztAwake()
        UIViewController.yztAwake()
...
extension UIApplication
{
    private static let runOnce:Void = { //使用静态属性以保证只调用一次(该属性是个方法)
        YZTNothingToSeeHere.harmlessFunction()
    }()
...
extension UINavigationBar:YZTAwakeProtocol
{
    ...
    fileprivate func yzt_setBackgroundImage(image:UIImage)
    {
        ...
        if (backgroundImageView == nil)
        {
            ...
            backgroundImageView = UIImageView(frame: CGRect(x: 0, y: 0, width: Int(bounds.width), height: YZTNavigationBar.navBarBottom()))
    ...
    fileprivate func yzt_setBackgroundColor(color:UIColor)
    {
        ...
        if (backgroundView == nil)
        {
            // add a image(nil color) to _UIBarBackground make it clear
            setBackgroundImage(UIImage(), for: .default)
            backgroundView = UIView(frame: CGRect(x: 0, y: 0, width: Int(bounds.width), height: YZTNavigationBar.navBarBottom()))
    ...
    fileprivate func yzt_setBackgroundAlpha(alpha:CGFloat)
    {
    ...
    open func yzt_setBarButtonItemsAlpha(alpha:CGFloat, hasSystemBackIndicator:Bool)
    {
    ...
    /// 设置导航栏在垂直方向上平移多少距离
    open func yzt_setTranslationY(translationY:CGFloat)
    {
        transform = CGAffineTransform.init(translationX: 0, y: translationY)
    }
    
    open func yzt_getTranslationY() -> CGFloat
    {
        return transform.ty
    }
    ...
    public static func yztAwake()
    {
        DispatchQueue.once(token: onceToken)
        {
            ...
            for selector in needSwizzleSelectorArr {
                let str = ("yzt_" + selector.description)
    ...
    @objc open func yzt_setTitleTextAttributes(_ newTitleTextAttributes:[String : Any]?)
    {
        ...
        guard let originTitleTextAttributes = titleTextAttributes else {
            yzt_setTitleTextAttributes(attributes)
            return
        }
...
extension UINavigationController: YZTFatherAwakeProtocol
{
    override open var preferredStatusBarStyle: UIStatusBarStyle {
        return topViewController?.statusBarStyle ?? YZTNavigationBar.defaultStatusBarStyle
    }
    
    fileprivate func setNeedsNavigationBarUpdate(backgroundImage: UIImage)
    {
        navigationBar.yzt_setBackgroundImage(image: backgroundImage)
    }
    
    fileprivate func setNeedsNavigationBarUpdate(barTintColor: UIColor)
    {
        navigationBar.yzt_setBackgroundColor(color: barTintColor)
    }
    
    fileprivate func setNeedsNavigationBarUpdate(barBackgroundAlpha: CGFloat)
    {
        navigationBar.yzt_setBackgroundAlpha(alpha: barBackgroundAlpha)
    }
    ...
    fileprivate func updateNavigationBar(fromVC: UIViewController?, toVC: UIViewController?, progress: CGFloat)
    {
        ...
        let fromBarTintColor = fromVC?.navBarBarTintColor ?? YZTNavigationBar.defaultNavBarBarTintColor
        let toBarTintColor   = toVC?.navBarBarTintColor ?? YZTNavigationBar.defaultNavBarBarTintColor
        let newBarTintColor  = YZTNavigationBar.middleColor(fromColor: fromBarTintColor, toColor: toBarTintColor, percent: progress)
        setNeedsNavigationBarUpdate(barTintColor: newBarTintColor)
        
        // change navBarTintColor
        let fromTintColor = fromVC?.navBarTintColor ?? YZTNavigationBar.defaultNavBarTintColor
        let toTintColor = toVC?.navBarTintColor ?? YZTNavigationBar.defaultNavBarTintColor
        let newTintColor = YZTNavigationBar.middleColor(fromColor: fromTintColor, toColor: toTintColor, percent: progress)
        setNeedsNavigationBarUpdate(tintColor: newTintColor)
        
        // change navBarTitleColor
        //        let fromTitleColor = fromVC?.navBarTitleColor ?? YZTNavigationBar.defaultNavBarTitleColor
        //        let toTitleColor = toVC?.navBarTitleColor ?? YZTNavigationBar.defaultNavBarTitleColor
        //        let newTitleColor = YZTNavigationBar.middleColor(fromColor: fromTitleColor, toColor: toTitleColor, percent: progress)
        //        setNeedsNavigationBarUpdate(titleColor: newTitleColor)
        
        // change navBar _UIBarBackground alpha
        let fromBarBackgroundAlpha = fromVC?.navBarBackgroundAlpha ?? YZTNavigationBar.defaultBackgroundAlpha
        let toBarBackgroundAlpha = toVC?.navBarBackgroundAlpha ?? YZTNavigationBar.defaultBackgroundAlpha
        let newBarBackgroundAlpha = YZTNavigationBar.middleAlpha(fromAlpha: fromBarBackgroundAlpha, toAlpha: toBarBackgroundAlpha, percent: progress)
    ...
    public static func fatherAwake()
    {
        DispatchQueue.once(token: onceToken)
        {
            ...
            for selector in needSwizzleSelectorArr {
                // _updateInteractiveTransition:  =>  yzt_updateInteractiveTransition:
                let str = ("yzt_" + selector.description).replacingOccurrences(of: "__", with: "_")
                if let originalMethod = class_getInstanceMethod(self, selector),
                    let swizzledMethod = class_getInstanceMethod(self, Selector(str)) {
                    method_exchangeImplementations(originalMethod, swizzledMethod)
                }
    ...
    @objc open func yzt_popToViewController(_ viewController: UIViewController, animated: Bool) -> [UIViewController]?
    {
        ...
        CATransaction.begin()
        let vcs = yzt_popToViewController(viewController, animated: animated)
    ...
    @objc open func yzt_popToRootViewControllerAnimated(_ animated: Bool) -> [UIViewController]?
    {
        ...
        CATransaction.setAnimationDuration(popProperties.popDuration)
        CATransaction.begin()
        let vcs = yzt_popToRootViewControllerAnimated(animated)
        CATransaction.commit()
        return vcs;
    }
    ...
    @objc open func yzt_pushViewController(_ viewController: UIViewController, animated: Bool)
    {
        ...
        CATransaction.begin()
        yzt_pushViewController(viewController, animated: animated)
        CATransaction.commit()
    }
...
extension UINavigationController: UINavigationBarDelegate
{
    ...
    private func dealInteractionChanges(_ context: UIViewControllerTransitionCoordinatorContext)
    {
        let animations: (UITransitionContextViewControllerKey) -> () = {
            let curColor = context.viewController(forKey: $0)?.navBarBarTintColor ?? YZTNavigationBar.defaultNavBarBarTintColor
            let curAlpha = context.viewController(forKey: $0)?.navBarBackgroundAlpha ?? YZTNavigationBar.defaultBackgroundAlpha
    ...
    @objc open func yzt_updateInteractiveTransition(_ percentComplete: CGFloat)
    {
        guard let topViewController = topViewController,
            let coordinator       = topViewController.transitionCoordinator else {
                yzt_updateInteractiveTransition(percentComplete)
                return
        }
...
//=============================================================================
extension UIViewController: YZTAwakeProtocol
{
    fileprivate struct AssociatedKeys
    {
        ...
        static var navBarBarTintColor: UIColor = YZTNavigationBar.defaultNavBarBarTintColor
        static var navBarBackgroundAlpha:CGFloat = 1.0
        static var navBarTintColor: UIColor = YZTNavigationBar.defaultNavBarTintColor
        static var navBarTitleColor: UIColor = YZTNavigationBar.defaultNavBarTitleColor
    ...
    open var navBarBackgroundImage: UIImage?
    {
        get {
            guard let bgImage = objc_getAssociatedObject(self, &AssociatedKeys.navBarBackgroundImage) as? UIImage else {
                return YZTNavigationBar.defaultNavBarBackgroundImage
            }
            return bgImage
        }
                set {
                    if customNavBar.isKind(of: UINavigationBar.self) {
                        ...
                        navBar.yzt_setBackgroundImage(image: newValue!)
                    }
                    else {
    ...
    open var navBarBarTintColor: UIColor {
        ...
        set {
            ...
            if customNavBar.isKind(of: UINavigationBar.self) {
                let navBar = customNavBar as! UINavigationBar
                navBar.yzt_setBackgroundColor(color: newValue)
            }
            else {
    ...
    open var navBarBackgroundAlpha:CGFloat {
        ...
        set {
            ...
            if customNavBar.isKind(of: UINavigationBar.self) {
                let navBar = customNavBar as! UINavigationBar
                navBar.yzt_setBackgroundAlpha(alpha: newValue)
            }
            else {
                if canUpdateNavBarBarTintColorOrBackgroundAlpha == true {
    ...
    private static let onceToken = UUID().uuidString
    @objc public static func yztAwake()
    {
        DispatchQueue.once(token: onceToken)
        {
            ...
            {
                let newSelectorStr = "yzt_" + selector.description
                if let originalMethod = class_getInstanceMethod(self, selector),
    ...
    func canUpdateNavigationBar() -> Bool
    {
        ...
        let middleFrame = CGRect(x: 0, y: YZTNavigationBar.navBarBottom(), width: YZTNavigationBar.screenWidth(), height: YZTNavigationBar.screenHeight()-YZTNavigationBar.navBarBottom())
        let minFrame = CGRect(x: 0, y: YZTNavigationBar.navBarBottom(), width: YZTNavigationBar.screenWidth(), height: YZTNavigationBar.screenHeight()-YZTNavigationBar.navBarBottom()-YZTNavigationBar.tabBarHeight())
